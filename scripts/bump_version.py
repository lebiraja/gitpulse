...
def main() -> None:
    ...
    # Anchor to the start of a line so we match the [project] version field,
    # not a version string that might appear inside a dependency specifier.
    ...
    # Update pyproject.toml — anchor pattern to start-of-line for safety
    update_file(
        "pyproject.toml",
        r'^(version\s*=\s*)"([^"]+)"',
        rf'\g<1>"{new_version}"',
